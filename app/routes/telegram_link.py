...
def create_client_for_user(user_id: int) -> TelegramClient:
    """
...
async def send_telegram_code(
    ...
    client = create_client_for_user(current_user.id)
...
async def confirm_telegram_code(
    ...
    try:
        ...
        if not me:
            ...
            raise HTTPException(status_code=500, detail="Не удалось получить данные Telegram-профиля")
            
        
        session_name = f"tg_user_{current_user.id}"

        await user_repo.update_user_telegram(current_user.id, session_name, me.username)
        await manager.add_session(session_name)
        del PHONE_HASH[current_user.id]
        return JSONResponse(
            {
                "ok": True,
...
async def confirm_telegram_password(
    ...
    if not phone or not password:
        raise HTTPException(status_code=400, detail="Номер телефона и облачный пароль обязательны")
