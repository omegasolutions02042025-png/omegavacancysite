...
'''
  ...
  def utochnenie_mail_promt(self, json_data: dict, username: str, vacancy_text: str):
    ...
    def extract_candidate_stack(candidate: dict) -> list[str]:
      """
    ...
    def build_questions_block(must_have_candidate: list[dict], candidate: dict) -> str:
      ...
      """
      candidate_stack_list = extract_candidate_stack(candidate)
      ...
      for requirement, status in uniq_issues:
          ...
          if missing_techs:
              if len(missing_techs) == 1:
                  ...
                  q = (
                      f"просим уточнить, есть ли  {tech}: "
    ...
    def mail_for_candidate_utochnenie(
        json_data: dict,
        vacancy_text: str,
        username: str,
    ):
      ...
      prompt = f"""
Ты — продвинутый ассистент IT-рекрутера компании Omega Solutions.  
Твоя задача — составить готовое письмо кандидату для уточнения деталей.

КРИТИЧЕСКИ ВАЖНО:
— Выводи ТОЛЬКО готовый текст письма, БЕЗ Markdown, БЕЗ кавычек, БЕЗ пояснений.
— Письмо должно быть структурированным, читаемым и профессиональным.
...
[ГОТОВЫЙ БЛОК УТОЧНЯЮЩИХ ВОПРОСОВ]
{questions_block if questions_block else "(блок пуст — не включать в письмо)"}

---

СТРУКТУРА ПИСЬМА (следуй строго):

1. ПРИВЕТСТВИЕ
Здравствуйте, {candidate.get('full_name', 'коллега')}!

2. БЛАГОДАРНОСТЬ И ПРЕДСТАВЛЕНИЕ ВАКАНСИИ
Компания Omega Solutions благодарит вас за предоставленное резюме и интерес к нашим вакансиям.
Мы рассматриваем вас на позицию {vacancy.get('position_name', 'позиция')} 🆔 {vacancy.get('position_id', '')} в рамках проекта нашего заказчика.

3. ОПИСАНИЕ ПРОЕКТА (если есть в тексте вакансии)
Извлеки 1-2 предложения из разделов «О проекте», «Про проект», «Описание проекта».
Используй дословные фразы. Если такого раздела нет — пропусти этот блок.

4. ЗАДАЧИ (если есть в тексте вакансии)
Извлеки 2-3 ключевые задачи из разделов «Задачи», «Обязанности», «Что делать».
Выводи дословно, по одному пункту в строку с маркером "—".
Если такого раздела нет — пропусти этот блок.

5. ФОРМАТ РАБОТЫ И УСЛОВИЯ
Укажи формат работы (удалённо/офис/гибрид), если указан в вакансии.
Упомяни особые условия (NDA, SIM-карта, график), если есть.
ВАЖНО: НЕ упоминай оплату, ставки, зарплату, бюджет.

6. ТЕХНОЛОГИЧЕСКИЙ СТЕК
Перечисли ключевые технологии из вакансии одной строкой через запятую.
Пример: "Основной стек: Python, Django, PostgreSQL, Docker, Kubernetes."

7. СООТВЕТСТВИЕ КАНДИДАТА
Напиши 1-2 предложения о том, какие навыки кандидата соответствуют вакансии.
Используй конкретные технологии из стека кандидата: {candidate.get('tech_stack', [])}
Пример: "Ваш опыт с Python, Django и PostgreSQL хорошо соответствует требованиям проекта."

8. БЛОК УТОЧНЯЮЩИХ ВОПРОСОВ
ЕСЛИ блок уточняющих вопросов НЕ ПУСТОЙ:
— Вставь его ДОСЛОВНО, БЕЗ ИЗМЕНЕНИЙ.
— Не переформулируй вопросы.
— Сохрани все формулировки и структуру.
ЕСЛИ блок ПУСТОЙ:
— Пропусти этот раздел полностью.
...
— Используй маркеры "—" для списков задач.
— Не используй Markdown-разметку (**, __, # и т.д.).
— Не используй эмодзи, кроме 🆔 для ID вакансии.
— Текст должен быть чистым, без лишних символов.

ПРАВИЛА ИЗВЛЕЧЕНИЯ ИНФОРМАЦИИ:
— Извлекай информацию ДОСЛОВНО из текста вакансии.
— Не перефразируй и не сокращай требования.
— Если информации нет — пропускай соответствующий блок.
...
Компания Omega Solutions благодарит вас за предоставленное резюме и интерес к нашим вакансиям.
Мы рассматриваем вас на позицию Разработчик-эксперт BPMSoft 🆔BD-10261 в рамках долгосрочного проекта по замещению CRM B2C.

Проект направлен на создание современной системы управления клиентскими отношениями для крупного заказчика.
...
Формат работы: удалённая полная занятость.

Основной стек: BPMSoft, C#, Ext JS, Angular, Kafka, REST API.

Ваш опыт с BPMSoft, C#, Ext JS и Angular хорошо соответствует требованиям вакансии.

ЧТОБЫ ОЦЕНТЬ ВАШЕ ПОЛНОЕ СООТВЕТСТВИЕ ТРЕБОВАНИЯМ ВАКАНСИИ, ПОЖАЛУЙСТА, УТОЧНИТЕ:
— просим уточнить, есть ли у вас практический опыт работы с Kafka: на каких проектах вы его использовали и какие задачи решали
— просим уточнить ваш практический опыт работы со следующими технологиями: Swagger, Postman, GitLab (проекты, задачи, роль)
...
"""
      return prompt
    prompt = mail_for_candidate_utochnenie(json_data, vacancy_text, username)
    return prompt
...
— Если в тексте встречаются русские или английские формулировки, сопоставляй их так:
  ...
  * "junior+", "junior +", "джуниор+", "джун+"                        → "Junior+"
  * "middle", "mid", "средний"                                        → "Middle"
  * "middle+", "mid+", "middle +", "mid +", "средний+"               → "Middle+"
  * "senior", "sr", "сеньор", "старший"                               → "Senior"
  * "senior+", "sr+", "senior +", "сеньор+", "старший+"              → "Senior+"
  * "lead", "team lead", "tech lead", "ведущий", "лид"                → "Lead"
— Если в тексте указан грейд с "+" (например, "middle+", "junior+", "senior+"), обязательно используй вариант с "+".
