...
Запуск через: python -c "from app.setup_admin import create_admin_sync; create_admin_sync()"
"""
import asyncio
...
async def create_admin_account():
    ...
    if admin:
        ...
        print(f"\n💾 Учетные данные сохранены в: {credentials_file.absolute()}")
        print(f"\n🌐 URL для входа:")
        print(f"   - http://localhost:8000/admin/login")
        print(f"   - http://omegahire.tech/admin/login")
...
def create_admin_sync():
    """Синхронная обертка для создания администратора"""
    return asyncio.run(create_admin_account())
